...
3. Observe runtime only with read-only commands chosen by what the arguments point to:
   - Swarm/service: `docker service ls`, `docker service ps`, `docker service logs`, `docker node ls`, or `task swarm:logs STACK=<stack> SERVICE=<service>` when a specific service is known
